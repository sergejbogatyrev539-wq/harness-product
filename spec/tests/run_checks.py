...
def make_isolation(reference) -> dict:
    ...
    profile = {
        ...
        "network": {"worker_default": "DISCONNECTED", "loopback": "DISABLED", "ipv4": "DISABLED", "ipv6": "DISABLED", "dns": "DISABLED", "unix_sockets": "NONE", "metadata_service": "BLOCKED", "inherited_fds_closed": True},
...
def make_candidate(reference, manifest: dict, policy: dict, isolation: dict) -> dict:
    ...
    ipc = {"attestation_digest": digest("9"), "isolation_profile_digest": isolation["profile_digest"], "broker_ipc_binding_digest": isolation["broker_ipc_binding"]["binding_digest"], "runtime_session_id": session["session_id"], "worker_principal": worker["principal"], "broker_principal": broker["principal"], "worker_subject": deepcopy(worker["os_subject"]), "broker_subject": deepcopy(broker["os_subject"]), "socket_identity": {"kind": "UNIX_PATH_INODE", "mount_id": "mount:ipc", "device_id": "device:ipc", "inode": 7}, "worker_fd": 3, "broker_fd": 4, "worker_fd_allowlist_digest": digest("1"), "broker_fd_allowlist_digest": digest("2"), "worker_observed_peer": {key: broker["os_subject"][key] for key in ("process_id", "uid", "gid", "session_id")}, "broker_observed_peer": {key: worker["os_subject"][key] for key in ("process_id", "uid", "gid", "session_id")}, "transport": "UNIX_SEQPACKET", "message_schema_digest": digest("3"), "operation_id": "write-file", "nonce": "broker_ipc_nonce_123456789", "session_fencing_epoch": 3, "issued_at": "2026-08-24T00:00:00Z", "expires_at": "2026-08-24T00:10:00Z", "revocation_epoch": 3, "signature": {"key_id": "key:broker-ipc", "algorithm": "ED25519", "payload_digest": digest("4"), "verified_by": "verifier:broker-ipc"}}
...
def check_canonical_chain(reference) -> tuple[dict, dict, dict, dict, dict, dict, dict]:
    ...
    for name, mutate in (("MISSING-SOCKET-FIELD", lambda x: x["broker_ipc_attestation"]["socket_identity"].pop("inode")), ("ABSTRACT-NAMESPACE", lambda x: x["broker_ipc_attestation"].update(socket_identity={"kind": "UNIX_ABSTRACT", "network_namespace": "net:forged", "abstract_name": "broker"})), ("OPERATION", lambda x: x["broker_ipc_attestation"].update(operation_id="other-operation"))):
...
def check_approval_isolation(reference, isolation, facts) -> int:
    ...
    for name, mutate in (("ROOTLESS", lambda p: p["process"].update(rootless_user_mapping=False)), ("NETWORK", lambda p: p["network"].update(dns="BROKER_ONLY")), ("IPC-PRINCIPAL", lambda p: p["broker_ipc_binding"].update(broker_principal="broker:other")), ("IPC-ENDPOINT", lambda p: p["broker_ipc_binding"].update(worker_endpoint="ipc:other")), ("IPC-TRANSPORT", lambda p: p["broker_ipc_binding"].update(transport="STREAM")), ("IPC-PEERCRED", lambda p: p["broker_ipc_binding"].update(peer_credentials_verified=False))):
    ...
    for field, value in (("worker_default", "DISABLED"), ("loopback", "NONE"), ("ipv4", "BLOCKED"), ("unix_sockets", "DISABLED"), ("metadata_service", "NONE")):
