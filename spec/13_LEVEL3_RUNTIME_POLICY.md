...
Broker IPC verifier facts are likewise not boolean: admission requires one externally verified signed complete-payload record for the exact worker↔broker `UNIX_SEQPACKET` session, including profile and IPC-binding digests, subjects, socket identity, FD allowlists, observed peers, message schema, operation, nonce, fence, lifetime and revocation. The canonical signed payload excludes only `attestation_digest` and `signature`; `attestation_digest` and `signature.payload_digest` both equal its canonical digest. Its attestation and IPC-binding digests continue unchanged through decision, capability, durable dispatch, receipt/event and `JOIN`; durable dispatch and `JOIN` recheck the record. Reconnect or socket replacement is material change requiring fresh admission, not a transition or service.
