...
| `L0-REQ-008` | Перед exec supervisor MUST закрыть все FDs кроме explicit typed allowlist (`close_range`/equivalent), запретить broad inherited sockets/pipes, shared memory и host IPC. Для LX-A единственный connected FD worker-а — exact operation-scoped `UNIX_CONNECTED_PAIR` proposal endpoint; external attestor до start gate связывает оба endpoint-FD с точными worker/broker subjects, FD inventories и policy digests, а broker проверяет per-message credentials и closed payload. | Supervisor + IPC broker + external attestor | Start refusal; kill/revoke on unexpected/substituted endpoint, holder, credential or FD | `T-ATTACK-L0-IPC-001`, FD/endpoint/message-credential inventory |
| `L0-REQ-009` | `L0-LX-A/B` worker MUST быть network-default-disconnected: IPv4, IPv6, loopback, routes, DNS resolver, metadata endpoints, raw/packet sockets и direct UNIX-socket creation/connect недоступны. Единственное исключение — preconnected exact broker proposal pair из `L0-REQ-008`; pathname, abstract, socketpair/reconnect и любой второй connected FD запрещены. | Host network/runtime/seccomp | Socket/connect blocked; authoritative event; kill on bypass | `T-ATTACK-L0-NET-001/002`, packet/syscall/route/FD evidence |
| `L0-REQ-010` | Любой разрешённый внешний flow MUST выполняться отдельным broker/executor/gateway principal с own L0 profile и exact-bound capability. Exact proposal pair не является sink authority: worker MUST NOT получать connected external sink, executor/gateway endpoint или credential, а broker pair MUST NOT передавать FD/credential и не может расширять proposal authority. | Broker + executor | `DENY`; revoke route | `T-ATTACK-L0-BROKER-001`, cross-principal trace |
...
The worker MUST have rootless user mapping, a non-root externally verified UID/GID, and disabled loopback, IPv4, IPv6 and DNS; it has no worker network path or generic Unix-socket allowance. Its only broker interaction is a separately attested closed `broker_ipc_binding`: exact worker/broker principals and endpoints, their distinct named network namespaces, `UNIX_SEQPACKET` transport, `endpoint_mode=UNIX_CONNECTED_PAIR`, typed supervisor delivery and `SCM_CREDENTIALS_PLUS_ENDPOINT_HOLDER_ATTESTATION`. Before either operation gate opens, an external attestor binds both immutable endpoint identities, exact FD numbers/allowlists, current holders, sessions, labels, cgroups and namespaces. Broker accepts one bounded canonical message only when its per-message credentials and all operation/nonce/fence/lifetime bindings match; ancillary FD/credential substitution, extra connected FD, pathname/abstract socket, reconnect or pair replacement prevents start or quarantines the session. Creation-time `SO_PEERCRED` is recorded only as provenance and is not post-exec peer proof. This pair carries a powerless proposal and is not an external sink, executor route or exception to the worker's external-network isolation.
